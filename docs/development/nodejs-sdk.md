...
官方发布仓库在 `sdk/nodejs` 提供正向与反向 WebSocket SDK。本地托管插件由框架分配服务连接并管理生命周期；同一业务代码可以保留 WS 适配层，以便独立部署。
...
- 本地托管包需声明运行入口和管理端能力，并通过运行时上下文检查管理 API 版本与 action 能力；
- 令牌通过环境变量或受保护配置注入；
- 连接成功后先读取插件上下文；
...
- 本地托管与正向/反向 WebSocket 使用同一套 action、事件和权限语义；
