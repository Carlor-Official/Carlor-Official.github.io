...
官方发布仓库在 `sdk/nodejs` 提供原生 IPC、正向与反向 WebSocket SDK。原生插件由框架启动并管理生命周期；同一业务代码可以保留 WS 适配层，以便独立部署。
...
- 原生托管包需声明 `native-ipc-v1`、运行入口、最低框架版本和能力清单，并通过运行时上下文检查管理 API 版本与 action 能力；
- 令牌通过环境变量或受保护配置注入；
- 连接成功后先读取插件上下文；
