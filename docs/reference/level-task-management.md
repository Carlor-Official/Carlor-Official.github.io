...
“听歌领金币兑换加速”不增加公开参数。QQ 音乐的余额查询结果不作为执行前置条件，框架以兑换接口的业务结果判断成功或金币不足，避免部分账号余额接口返回 `0` 时被误拦截；调用方仍只按任务标题执行，并以刷新后的 QQ 等级面板判断完成状态。框架会短时复用同一账号的 QQ 音乐登录 Cookie，并在上游返回 `100050` 时进入 10 分钟冷却，避免刷新或重复点击继续触发换码限流。
