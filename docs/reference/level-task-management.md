...
- [获取等级加速账号列表](/api/get_level_task_accounts.html)：`get_level_task_accounts`
- [获取账号等级进度](/api/get_level_task_account.html)：`get_level_task_account`
- [获取完整等级任务面板](/api/get_level_task_panel.html)：`get_level_task_panel`
- [读取等级任务计划](/api/get_level_task_settings.html)：`get_level_task_settings`
- [保存等级任务计划](/api/update_level_task_settings.html)：`update_level_task_settings`
- [执行等级任务并刷新面板](/api/execute_level_task_selection.html)：`execute_level_task_selection`
...
`电脑QQ在线` 已支持手动补挂和框架定时计划。目标仍传在线 Android 账号的 `self_id` 和 `client_type: 'android'`；框架使用同 QQ Android 会话授权 Linux `3.2.32` 免扫登录。该任务与账号页共用同一 Linux 二维码、扫码、授权、查询链路，不使用 Android 登录包替代 Linux 登录。已有 Linux 在线会话直接复用，已有账号的节点和设备不被覆盖；没有 Linux 账号时，框架生成独立设备配置并沿用 Android 账号的登录节点创建 Linux 账号。
...
“听歌领金币兑换加速”不增加公开参数。v2.4.4 起，QQ 音乐的余额查询结果不作为执行前置条件，框架以兑换接口的业务结果判断成功或金币不足，避免部分账号余额接口返回 `0` 时被误拦截；调用方仍只按任务标题执行，并以刷新后的 QQ 等级面板判断完成状态。框架会短时复用同一账号的 QQ 音乐登录 Cookie，并在上游返回 `100050` 时进入 10 分钟冷却，避免刷新或重复点击继续触发换码限流。
