...
萌卡 NT v2.4.0 支持“框架内本地托管”插件：用户从「插件 → 插件导入」上传可信成品包，框架在本地插件目录中安装、启动并管理它。托管插件通过框架分配的服务连接调用 API；需要独立部署时，仍可使用正向或反向 WebSocket。

## 选择连接模式

| 模式 | 连接发起方 | 适用场景 |
| --- | --- | --- |
| 本地托管 | 框架启动插件并管理本地服务连接 | 推荐的 v2.4.0 使用方式 |
| 正向 WebSocket | 插件连接萌卡 NT | 独立进程或跨主机部署 |
| 反向 WebSocket | 萌卡 NT 连接插件 | 插件有稳定可访问地址 |

两种模式使用相同的 API 和事件数据。区别只在连接方向与初始化方法。

推荐让同一个插件同时支持本地托管、正向 WS 和反向 WS。用户上传后由框架生成运行目录、数据目录和生命周期控制；外部部署则在「插件 → 插件对接」中创建正向或反向 WS 服务。当前不再依赖官网插件清单、远程下载或市场审核。
...
正向模式还需要监听端口；反向模式需要 WebSocket 地址和重连间隔。本地托管插件无需手动填写外部端口，由框架在导入时分配服务连接。
