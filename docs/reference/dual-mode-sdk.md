...
完整例子位于 [`examples/dual-mode/server.mjs`](https://github.com/Carlor-Official/Mengka-NT/blob/main/sdk/nodejs/examples/dual-mode/server.mjs)。它同时演示双向 WS、手动管理员认证、托管管理员落盘、重复启动、健康检查、激活和停止。示例手动管理端仅监听本机，使用 HTTP Basic；正式产品可替换为自己的密码哈希与 Session 登录，远程代理时使用 TLS。
