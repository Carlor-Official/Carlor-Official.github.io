...
::: tip v2.4.4 协议基线
Linux QQ 仅保留 `3.2.32`。扫码登录、同 QQ Android 免扫授权和等级任务“电脑QQ在线”共用同一 Linux 二维码会话；`3.2.33` 及其算法路由已移除。业务 API 的公开参数不变，框架内部按实际协议字段组包。“听歌领金币兑换加速”以兑换接口业务结果为准，不再由 QQ 音乐余额查询结果提前拦截。
:::
